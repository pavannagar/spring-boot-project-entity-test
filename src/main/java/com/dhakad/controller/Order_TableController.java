...
public class Order_TableController {
	...
	public int changeStatus(@RequestBody Order_Table order_Table) {
		return order_TableService.changeStatus(order_Table.getOrderId(), order_Table.getStatus());
	}

	@GetMapping("order/ab/{id}")
	public List<Object[]> getOrderStatus(@PathVariable("id") int orderId){
		return order_TableService.getOrderStatus(orderId);
	}
